...
export class MapViewModel extends BaseViewModel<MapState> {
  ...
  async loadAllPOIs(): Promise<void> {
    try {
      ...
      this.setError(error instanceof Error ? error : new Error('Failed to load POIs'));
    } finally {
  ...
  async calculateRoute(mode: TransportationMode = TransportationMode.WALKING): Promise<void> {
  ...
  async switchToIndoorMode(building: Building, floor: Floor): Promise<void> {
    try {
      ...
      this.setError(error instanceof Error ? error : new Error('Failed to switch to indoor mode'));
    } finally {
      this.setLoading(false);
    }
  }
